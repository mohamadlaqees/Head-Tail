...
head.onclick=function(){
    ...
    setTimeout(() => {
        ...
        let path=`url(../images/${imge[Math.floor(Math.random() * imge.length)]})`;
        img.style.backgroundImage=path;
        if(path==='url(../images/head.png)' && cchoise.innerHTML==='head' ){
            pnum++;
            pnume.innerHTML=pnum;
            cnum++;
            cnume.innerHTML=cnum;
        }
        else if(path==='url(../images/head.png)' && cchoise.innerHTML!='head'){
            pnum++;
            pnume.innerHTML=pnum;
        }
        else if(path==='url(../images/tail.png)' && cchoise.innerHTML==='tail'){
            cnum++;
            cnume.innerHTML=cnum;
        }
    }, 2000);
...
tail.onclick=function(){
    ...
    setTimeout(() => {
        ...
        let path=`url(../images/${imge[Math.floor(Math.random() * imge.length)]})`;
        img.style.backgroundImage=path;
        if(path==='url(../images/tail.png)' && cchoise.innerHTML==='tail'){
            pnum++;
            pnume.innerHTML=pnum;
            cnum++;
            cnume.innerHTML=cnum;
